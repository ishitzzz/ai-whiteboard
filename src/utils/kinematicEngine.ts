import type { RawCoordinate, KinematicCoordinate } from '../types/engine';

/**
 * The Kinematic Processing Engine ("The Wrist")
 * Transforms mathematically perfect, raw geometry into continuous, humanized motion
 * by calculating instantaneous velocity, overshoots, and varying pressure.
 */
export function applyHumanKinematics(rawPath: RawCoordinate[]): KinematicCoordinate[] {
  ...
  // We first map the raw coordinates to compute incoming angles and local curvature.
  ...
  for (let i = 0; i < enrichedPoints.length; i++) {
    ...
    // Physical human motion means high velocity on straight lines and
    // dramatic deceleration in tight curves or dense intersections.
    ...
    // Strictly clamp velocity between user-defined limits: 0.5 and 5.0 pixels/frame
    velocity = Math.max(0.5, Math.min(5.0, velocity));
    
    // Map velocity to pressure inversely
    // A fast velocity (5.0) yields low pressure/thin lines (~0.3)
    // A slow velocity (0.5) yields high pressure/thick bleed (1.0)
    ...
    if (i > 0 && i < enrichedPoints.length - 1) {
      ...
      if (next.commandType !== 'moveTo') {
        ...
        if (angleDiff > Math.PI / 4) {
          isSharpCorner = true;
          // Use pseudo-random based on index to ensure deterministic rendering during stream
    ...
    // If it's a sharp corner, inject hesitation frames to force the renderer to linger
    if (isSharpCorner) {
      ...
      for (let j = 0; j < lingeringFrames; j++) {
        result.push({
          x: p.x,
          y: p.y,
          velocity: 0.05, // near-zero velocity forces rendering loop to pause
          curvature: p.curvature,
          overshootX: 0,
          overshootY: 0,
          pressure: 1.0 // Maximum marker bleed when hand pauses
        });
      }
    }
  }
